...
class Database
{
    private static $dbName = 'personal_learning';
    ...
    public function myPrepare(string $sql)
    {
        self::connect();
    ...
    {
        self::connect();
        if(empty($values)) $prep_pdoStatment->execute();
        else $prep_pdoStatment->execute($values);
        self::disconnect();
