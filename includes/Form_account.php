...
class Form_account
{
	...
	public function inputPassword(string $name, $needed=true) {
		$this->neededs += array($name=>$needed);
		// $this->inputs[] = $name;
		return "<div class='pswd-voir'><label for='{$name}'>{$name}</label><input type='password' id='{$name}' name='{$name}' placeholder='{$name} ...' ><div id='btnVoir' class='btn btn-voir'>Voir</div></div>";
	}
	// SUBMIT
	...
	///////////////////////////////////////////// CONNEXION /////////////////////////////////////////////
	public function signIn(/*$redirection ('typeUser0' => 'URL0')*/) { // la key sera recup de la BDD OU URL dans la BDD ???---------------------------------->
		if ( (count($this->dataform) != 0) && (count($this->dataform) == 2) ) {
			...
			if ($retour && $res = $retour->fetchAll()) {
				foreach ($res as $row) { // LOGIN UNIQUE ???? => boucle inutile --------------------->
						...
						} else echo "Erreur d'identifiant";
				}
			} else echo "Erreur d'identifiant ...";
